...
$(function () {
    ...
    var sentenceBlock = $('#sentence-block');
    ...
    var wordgaps = [];
    var current_sentence_is_checked = false;
    ...
    function check() {
        // TODO
        var gaps = [];
        wordgaps.forEach(function (word) {
            if(word.is_gap) {
                var answer = $('#wo' + word.order).val();
                gaps.push({
                    'answer': answer,
                    'order': word.order
                });
        ...
        $.ajax({
            ...
            error: function () {
                console.error('Cannot check results')
            }
        });
    }
    ...
    function replay() {
        ...
        replays = parseInt(numberOfRepetitionsSelect.val());
        delay = parseInt(delayBetweenRepetitionsSelect.val());
    ...
    function lineIdChanged(lineId) {
        ...
        current_sentence_is_checked = false;

    }

    function clearInputAndResult() {
        $(sentenceBlock).find('input').val('');
    }

    function createWordGaps() {
            ...
            success: function (data) {
                ...
                wordgaps.forEach(function (word) {
                    if(word.is_gap) {
                        ...
                        var input = '<div class="gap">' +
                            '<span id="wa' + word.order + '"></span>' +
                            '<input id="wo' + word.order + '" />' +
                            '</div>';
                        sentenceBlock.append(input);
                    } else {
                        var span = '<span id="wo' + word.order + '" >' + word.word + '</span>';
                        sentenceBlock.append(span);
                    }
                });
                sentenceBlock.append('<span>.</span>');
                
                wordgaps.forEach(function (word) {
                    $('#wo' + word.order).mouseenter(showDefinition).mouseleave(hideDefinition);
                });


            },
            error: function () {
                console.error('Cannot get wordgaps data');
            }
        });
    }
    
    function showDefinition(event) {
        var hovered_id = $(event.currentTarget).attr('id');
        var order = parseInt(hovered_id.substr(2), 10);
        if (!current_sentence_is_checked && wordgaps[order - 1].is_gap) {
            return;
        }
        var definition = wordgaps[order - 1].definition;
        var original = wordgaps[order - 1].word;
        $('#definition-title').text(original);
        $('#definition-block').text(definition);
    }
    ...
    function setResult(result) {
        result.gaps.forEach(function (gap) {
            var gapDom = $('#wo' + gap.order);
            var correctDom = $('#wa' + gap.order);
            if(gap.correct) {
                gapDom.removeClass('error-input');
                gapDom.addClass('correct-input');
                correctDom.text('');
            } else {
                gapDom.removeClass('correct-input');
                gapDom.addClass('error-input');
                correctDom.text(gap.original);
            }
        });

        current_sentence_is_checked = true;
    }
