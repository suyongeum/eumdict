...
$(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                words.forEach(function (word) {
                    var span = $('<span id="wor' + word.order + '" >' + word.original + '</span>');
                    span.draggable();
                    wordsBlock.append(span);
                });
                sentenceBlock.append('<span>.</span>');
                
                words.forEach(function (word) {
                    $('#wor' + word.order).mouseenter(showDefinition).mouseleave(hideDefinition);
                });


            },
            error: function () {
                console.error('Cannot get words data');
            }
        });
    }
    ...
    function showDefinition(event) {
        ...
        var order = parseInt(hovered_id.substr(3), 10);
        var definition = words[order - 1].definition;
        var original = words[order - 1].original;
